      ...
      <div className=' text-black container mx-auto '>
        ...
        <header className='flex items-center justify-between p-6'>
          <div className='flex items-center space-x-2'>
            <img href="./" src={logo} alt="" className='h-20 w-20 rounded-full ' />
            <h1 className='text-2xl text-indigo-950 font-bold'>CourseFunda</h1>
          </div>
          <div className='space-x-5'>
            {isLoggedIn ? (
              <button onClick={handleLogout} className="px-6 py-3 border border-indigo-600 rounded hover:bg-indigo-600 hover:text-white">Logout</button>
            ) : (
              <>
                <Link to={"/login"} className="px-6 py-3 border border-indigo-600 rounded hover:bg-indigo-600 hover:text-white">Login</Link>
                <Link to={"/signup"} className="px-6 py-3 border text-white bg-indigo-600 border-indigo-600 rounded hover:bg-transparent hover:text-black">Sign Up</Link>
              </>
            )}
          </div>
        </header>
        ...
        <section className='text-center py-20'>
          ...
          <div className='space-x-4 mt-6'>
            <Link to={"./courses"} className='bg-indigo-900 hover:bg-neutral-50 text-white  hover:text-black transition duration-300 px-6 py-3 border border-indigo-400  rounded font-semibold'>Explore Courses</Link>
            <Link to="/alumni" className="bg-neutral-50 hover:bg-indigo-900 hover:text-white transition duration-300 px-6 py-3 border border-indigo-400 rounded font-semibold">
              Placed Alumni
            </Link>
        ...
        <section>
          <Slider {...settings}>

            {
              courses.map((course) => (
                <div>
                  <div key={course._id} className='bg-white rounded-lg shadow-lg p-4 m-4'>
